        ...
        <div class="tools" >
          ...
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="(item,index) in $store.state.sysRole" @click.native="changeSubMenu(item.name)" >{{item.meta.title}}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
    ...
    <el-col class="main" :span="24">
      ...
      <aside :class="collapsed?'menu-collapsed':'menu-expanded'" :style="{width:layerout.asideWidth+'px',height:this.layerout.mainHiehgt + 'px'}">
        ...
        <el-menu router
                 ...
                 @close="handleclose"
                 @select="handleselect"
                 v-if="!collapsed" unique-opened>

          <template v-for="(item , index) in asyncSubMenu"  v-if="!item.hidden">
            <el-submenu :index="index+''" v-if="!item.leaf">
              <template slot="title"><i :class="item.meta.iconCls"></i>{{item.meta.title}}</template>
              <el-menu-item v-for="child in item.children" :key="child.path" :index="child.path">{{child.meta.title}}</el-menu-item>
            </el-submenu>
            <!-- 叶子结点 -->
            <el-menu-item v-if="item.leaf&&item.children.length>0" :index="item.children[0].path">
              <i :class="item.meta.iconCls"></i>{{item.children[0].meta.title}}
            </el-menu-item>
          </template>
        </el-menu>
        ...
        <ul class="el-menu" v-else ref="menuCollapsed">
          <li v-for="(item,index) in asyncSubMenu" v-if="!item.hidden" class="reng-menu">
            ...
            <template v-else>
              ...
              <ul class="reng-submenu" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)">
                <li class="el-menu-item" v-for="child in item.children" :class="$route.path==child.path?'is-active':''" :key="child.path" @click="$router.push(child.path)" style="padding: 0 20px;">{{child.meta.title}}</li>
              </ul>
            </template>
          </li>
        </ul>
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      ...
      resetwindow(){
        this.layerout.mainHiehgt = this.$local.windowSize().height - 60;
        this.layerout.contentHeight = this.$local.windowSize().height -100;
      },
      changeSubMenu(sysname){
        this.$store.commit('changeSubMenuList',{name:sysname})
      }
    },
    created(){
      //let vm = this;
...
//        .then(data => {
//            vm.$store.dispatch('getuserInfo',data)  //更新用户信息
//            return data
//        }).then(data=>{
//          //this.$store.role
//        });
    ...
    computed:{
      ...
      avatarUrl(){
        return this.$store.state.profile.avatar_url
      },
      asyncSubMenu(){
        return this.$store.state.subMenuList.length>0 ? this.$store.state.subMenuList[0].children : []
      }
